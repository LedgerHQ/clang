// RUN: %clang_cc1 -frwpi-lowering -verify -emit-llvm -o /dev/null %s
...
extern const int * const h;
             ...
             int * const f = &extern_int;       // expected-warning {{embedded position-independent lowering of f made it non-const, other translation units will access it incorrectly}}
       const int *       g = &extern_const_int; // no-warning
       const int * const h = &extern_const_int; // no-warning
...
extern const func_ptr l;
       ...
       const func_ptr l = &extern_func; // no-warning
